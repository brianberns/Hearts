...
module Trainer =
    ...
    /// standard.
    let evaluate settings iteration epochOpt model =
    ...
    let trainModel settings iteration sampleStore =
        ...
        let eval epoch model =
            evaluate settings iteration (Some epoch) model
        AdvantageModel.train
            settings iteration (Some eval) sampleStore model
        stopwatch.Stop()
        ...
        Path.Combine(
            ...
            $"AdvantageModel%03d{iteration}.pt")
                |> model.save
                |> ignore
